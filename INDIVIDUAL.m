...
%--------------------------------------------------------------------------
    ...
    methods
        %% Constructor
        function obj = INDIVIDUAL(Decs,AddProper,nnstruct)
        ...
        %       H = INDIVIDUAL(rand(100,10),randn(100,3))
            ...
            if (nargin > 0) && (exist('nnstruct','var') == 0)
                % Create new objects
            ...
            elseif exist('nnstruct','var') == 1
                ...
                % Calculte the objective values and constraint
                % violations ！！！！ with neural network
                Decs = Global.problem.CalDec(Decs);
                %Objs = Global.problem.CalObj(Decs);
                Objs = RBF_predictor(nnstruct.W,nnstruct.B,nnstruct.Centers,nnstruct.Spreads,Decs);
                Cons = Global.problem.CalCon(Decs);
                % Assign the decision variables, objective values,
                % constraint violations, and additional properties
                for i = 1 : length(obj)
