...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        %       obj.NotTermination(Population)
            ...
            assert(notermination,'GLOBAL:Termination','Algorithm has terminated');%如果 cond 为 false，assert(cond,msg) 会引发错误并显示错误消息 msg。
            tic;
        end
        ...
        function Output(obj)
            ...
            if obj.evaluated >= obj.evaluation
                if obj.save == 0
                    ...
                    Metrics1 = {@HV};
                    Metrics2 = {@IGD};
                    metric.HV  = cellfun(@(S)GLOBAL.Metric(S,Population,obj.PF),Metrics1,'UniformOutput',false);
                    metric.IGD  = cellfun(@(S)GLOBAL.Metric(S,Population,obj.PF),Metrics2,'UniformOutput',false);
