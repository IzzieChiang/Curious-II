function [ W2,B2,Centers,Spreads ] = RBFNov( SamIn,SamOut,Nc)
...
SamIn=SamIn';
SamOut=SamOut';
SamNum = size(SamIn,2); %采样样本数,要作为center的选项
InDim = size(SamIn,1);  %每个样本的输入维度
ClusterNum = Nc; % InDim<Nc<SamNum
Overlap = 1.0; %正则化
%K-fold setup  %Random indexes
A=randperm(SamNum);
...
while 1
    ...
    if EqualNum == InDim*ClusterNum|ik>=5 %—————————————————防止过拟合———————————————————
        break;
    end
    ik=ik+1;
end

AllDistances = dist(Centers',Centers); %加权了
